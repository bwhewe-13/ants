...
def test_reed_backward_euler(boundary):
    xs_total, xs_scatter, xs_fission, external, boundary_x, medium_map, \
        delta_x, angle_x, angle_w, info, _ = problems1d.reeds(boundary)
    # Get Fixed Source
...
def test_sphere_01_backward_euler():
    info, PATH = problems1d.sphere_01("timed")[-2:]
    flux = timed1d.backward_euler(*problems1d.sphere_01("timed")[:-1])

...
def test_dimensions_external_unit():
    ...
    # Different boundary_x dimensions
    externals = [
        np.ones((info["cells_x"], info["cells_y"], 1, 1)),
        np.ones((info["cells_x"], info["cells_y"], 1, info["groups"])),
        np.ones(
            (info["cells_x"], info["cells_y"], info["angles"] ** 2, info["groups"])
        ),
    ]
    flux = []

...
def test_backward_euler_01():
    ...
    approx = hybrid1d.backward_euler(
        ...
        medium_map,
        SpatialGrid(delta_x),
        QuadratureData(angle_x, angle_w),
        QuadratureData(angle_x, angle_w),
        HybridMapping(fine_idx, coarse_idx, factor),
        info,
        info,
    )
...
def test_slab_01_bdf1(angles_c, groups_c):
    ...
    approx = hybrid1d.backward_euler(
        initial_flux,
        CrossSections(xs_total_u, xs_scatter_u, xs_fission_u),
        CrossSections(xs_total_c, xs_scatter_c, xs_fission_c),
        velocity_u,
        velocity_c,
        external,
        boundary_x,
        medium_map,
        SpatialGrid(delta_x),
        QuadratureData(angle_xu, angle_wu),
        QuadratureData(angle_xc, angle_wc),
        HybridMapping(fine_idx, coarse_idx, factor),
        info_u,
        info_c,
    )

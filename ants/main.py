...
def energy_grid(grid, groups_fine, groups_coarse=None, optimize=True):
    ...
    edges_gidx_fine = _group_indexing(
        grid, len(edges_g) - 1, groups_fine, edges_data=edges_data, optimize=optimize
    )
    ...
    edges_gidx_coarse = _group_indexing(
        grid, groups_fine, groups_coarse, edges_data=edges_data, optimize=optimize
    )
...
# Called from cython
def _energy_grid(grid, energy_groups):
    _, edges_gidx = energy_grid(grid, energy_groups)
    return edges_gidx
...
def weight_spatial2d(weight_matrix, xs_total, xs_scatter, xs_fission):
    ...
    return (
        medium_map,
        np.array(new_xs_total),
        np.array(new_xs_scatter),
...
def weight_matrix2d(edges_x, edges_y, materials, N_particles=100_000, **kwargs):
    ...
    assert (
        ("triangles" in kwargs.keys())
        or ("rectangles" in kwargs.keys())
        or ("circles" in kwargs.keys())
    ), "Need at least one shape"
    # Create uniform samples
    samples = np.random.uniform(
        size=(N_particles, 2), low=[0, 0], high=[np.max(edges_x), np.max(edges_y)]
    )
    # Create weight matrix (ii x jj x materials)
    weight_matrix = np.zeros((edges_x.shape[0] - 1, edges_y.shape[0] - 1, materials))
    ...
    for x, y in zip(samples[:, 0], samples[:, 1]):
        ...
        if "triangles" in kwargs.keys() and np.sum(tally) == 0.0:
            _inside_triangle(kwargs["triangles"], x, y, tally, kwargs["triangle_index"])
        if "rectangles" in kwargs.keys() and np.sum(tally) == 0.0:
            _inside_rectangle(
                kwargs["rectangles"], x, y, tally, kwargs["rectangle_index"]
            )
        if "circles" in kwargs.keys() and np.sum(tally) == 0.0:
            _inside_circle(kwargs["circles"], x, y, tally, kwargs["circle_index"])
        tally[tally > 0.0] = 1.0
        if np.sum(tally) == 0:
            tally[-1] = 1
    ...
    if "circles" in kwargs.keys():
        weight_matrix = _quarter_symmetry(
            weight_matrix, kwargs["circles"], edges_x, edges_y
        )

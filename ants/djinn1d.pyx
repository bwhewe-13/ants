...
cdef double[:,:] multigroup_collect(double[:,:]& flux_guess, double[:,:]& xs_total, \
    ...
    tracked_flux = tools.array_3d(2 * MAX_POWER, info.cells_x, info.groups)
    ...
    while not (converged):
        ...
        print("Count: {:>3}\tKeff: {:.8f}".format(str(count).zfill(3), \
                keff[0]), end="\r")
        converged = (change < EPSILON_POWER) or (count >= 2 * MAX_POWER)
        count += 1
        
        # Add old flux and tracked flux
        flux_old[:,:] = flux[:,:]
        tracked_flux[count-2] = flux[:,:]
...
cdef double[:,:] source_iteration_collect(double[:,:]& flux_guess, \
        ...
        double[:]& angle_w, params info, int iteration, str file):

...
def x_scalar_sweep(neutron_flux_old, medium_map, xs_total, xs_scatter, \
    ...
    sweep = vacuum if params[2] == 0 else reflected
    converged = 0
    count = 1
    while not (converged):
        neutron_flux *= 0
...
def convergence(flux, flux_old, angle_weight, angular=False):
    if angular:
        flux = np.sum(flux * angle_weight[None,:],axis=1)
        flux_old = np.sum(flux_old * angle_weight[None,:],axis=1)
    ...
    if spatial_coef > 0:
        for cell in range(cells):
            ...
            edge_two = (xs_scatter[material] * neutron_flux_old[cell] \
                ...
                + edge_one * (abs(spatial_coef) - 0.5 * xs_total[material])) \
                * 1/(abs(spatial_coef) + 0.5 * xs_total[material])
            if params[1] == 2: # Diamond Difference
                neutron_flux[cell] += angle_weight * 0.5 * (edge_one + edge_two)
            elif params[1] == 1: # Step Method
        ...
        for cell in range(cells-1, -1, -1):
            ...
            edge_one = (xs_scatter[material] * neutron_flux_old[cell] \
                + external_source[ex_group_idx+ex_angle_idx*params[3]::params[4]*params[3]][cell] \
                + edge_two * (abs(spatial_coef) - 0.5 * xs_total[material])) \
                * 1/(abs(spatial_coef) + 0.5 * xs_total[material])
            if params[1] == 2: # Diamond Difference
                neutron_flux[cell] += angle_weight * 0.5 * (edge_one + edge_two)
            elif params[1] == 1: # Step Method
...
def x_angular_sweep(neutron_flux_old, medium_map, xs_total, xs_scatter, \
    ...
    while not (converged):
        ...
        for angle in range(angles):
            ex_angle_idx == 0 if params[4] == 1 else angle
            sweep(neutron_flux[:,angle], scalar_flux, medium_map, \
                  xs_total, xs_scatter, external_source, \
                  point_source[angle], spatial_coef[angle], \
                  dummy_angle_weight[angle], params, ex_group_idx, ex_angle_idx)
        change = convergence(neutron_flux, neutron_flux_old, angle_weight, True)
        converged = (change < INNER_TOLERANCE) or (count >= MAX_ITERATIONS)
        count += 1
        neutron_flux_old = neutron_flux.copy()
    return neutron_flux
    ...
    if spatial_coef > 0:
        for cell in range(cells):
            ...
            edge_two = (xs_matrix[material] * neutron_flux_old[cell] \
                ...
                * (abs(spatial_coef) - 0.5 * xs_total[material] \

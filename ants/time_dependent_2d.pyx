...
cdef double[:,:,:,:] multigroup_bdf1(double[:,:,:]& flux_guess, \
            ...
            double[:]& velocity, double[:]& source, double[:]& boundary_x, \
...
cdef double[:,:,:,:] hybrid_bdf1(double[:,:]& xs_total_vu, double[:,:]& xs_total_vc, \
            ...
            double[:]& source_u, double[:]& boundary_x, double[:]& boundary_y, \
            int[:]& medium_map, double[:]& delta_x, double[:]& delta_y, 
            double[:]& angle_xu, double[:]& angle_yu, double[:]& angle_wu, \
            double[:]& angle_xc, double[:]& angle_yc, double[:]& angle_wc, \
    ...
    for step in range(params_u.steps):
        ...
        flux_uncollided = si.multigroup_scalar(flux_uncollided, xs_total_vu, \
                zero_matrix, source_star, boundary_x, boundary_y, medium_map, \
                delta_x, delta_y, angle_xu, angle_yu, angle_wu, params_u)
        ...
        flux_collided = si.multigroup_scalar(flux_collided, xs_total_vc, \
                xs_scatter_c, source_c, zero_boundary, zero_boundary, medium_map, \
                delta_x, delta_y, angle_xc, angle_yc, angle_wc, params_c)

...
cdef double[:,:,:] multigroup_angular(double[:,:,:]& flux_guess, \
    ...
    # Initialize flux
    flux = tools.array_3d(params.cells + params.edges, params.angles, params.groups)
    flux_old = flux_guess.copy()
    flux_1g = tools.array_2d(params.cells + params.edges, params.angles)
    # Create off-scattering term
    off_scatter = tools.array_1d(params.cells + params.edges)
...
cdef void slab_ordinates_angular(double[:,:] flux, double[:,:] flux_old, \
    ...
    scalar_flux = tools.array_1d(params.cells + params.edges)
...
cdef void ordinates_scalar(double[:] flux, double[:] flux_old, \
        ...
        params1d params):
    if params.geometry == 1: # slab
        slab_ordinates_scalar(flux, flux_old, xs_total, xs_scatter, \
                              off_scatter, external, boundary, medium_map, \
                              delta_x, angle_x, angle_w, params)
    elif params.geometry == 2: # sphere
...
cdef void initialize_half_angle(double[:]& flux, double[:]& half_angle, \
    ...
    for cell in range(params.cells-1, -1, -1):
        mat = medium_map[cell]
        half_angle[cell] = (2 * angle_plus + delta_x[cell] * (external[cell] \

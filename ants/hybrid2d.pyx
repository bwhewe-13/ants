...
def backward_euler(double[:,:] xs_total_u, double[:,:] xs_total_c, \
    ...
    parameters._check_hybrid2d_uncollided(info_u, xs_total_u.shape[0])
    # Convert collided dictionary to type params
    info_c = parameters._to_params(params_dict_c)
    parameters._check_hybrid2d_collided(info_c, xs_total_u.shape[0])
...
cdef double[:,:,:,:] multigroup_bdf1(double[:,:]& xs_total_u, double[:,:]& xs_total_c, \
    ...
    bcx2 = 1 if info_u.bcdim_x < 4 else info_u.steps
    bcy2 = 1 if info_u.bcdim_y < 4 else info_u.steps
...
cdef double[:,:,:,:] multigroup_cn(double[:,:]& xs_total_u, double[:,:]& xs_total_c, \
    ...
    q_star = tools.array_1d(info_u.cells_x * info_u.cells_y \
                            * info_u.angles * info_u.angles * info_u.groups)
    # Initialize angular flux for previous time step
    flux_last_x = tools.array_4d(info_u.cells_x + 1, info_u.cells_y, \
                               info_u.angles * info_u.angles, info_u.groups)
    flux_last_y = tools.array_4d(info_u.cells_x, info_u.cells_y + 1, \
                               info_u.angles * info_u.angles, info_u.groups)
    # Initialize scalar fluxes
    flux_u = tools.array_3d(info_u.cells_x, info_u.cells_y, info_u.groups)
    flux_c = tools.array_3d(info_c.cells_x, info_c.cells_y, info_c.groups)
    ...
    for step in tqdm(range(info_u.steps), desc="CN     ", ascii=True):
        ...
        qq1 = 0 if info_u.qdim < 4 else step
        bcx1 = 0 if info_u.bcdim_x < 4 else step
        bcy1 = 0 if info_u.bcdim_y < 4 else step
        # Update q_star
        tools._time_source_star_cn(flux_last_x, flux_last_y, flux_u, xs_total_u, \
                xs_scatter_u, velocity_u, q_star, external_u[qqa::qq2], \
                external_u[qq1::qq2], medium_map, delta_x, delta_y, angle_xu, \
                angle_yu, 2.0, step, info_u)
        # Run hybrid method
        hybrid_method(flux_u, flux_c, flux_t, xs_total_vu, xs_total_vc, \
                      xs_scatter_u, xs_scatter_c, q_star, source_c, \
                      boundary_xu[bcx1::bcx2], boundary_yu[bcy1::bcy2], \
        ...
        mg._interface_angular(flux_last_x, flux_last_y, xs_total_vu, q_star, \
                boundary_xu[bcx1::bcx2], boundary_yu[bcy1::bcy2], medium_map, \
...
cdef double[:,:,:,:] multigroup_bdf2(double[:,:]& xs_total_u, double[:,:]& xs_total_c, \
    ...
    flux_last_1 = tools.array_4d(info_u.cells_x, info_u.cells_y, \
                            info_u.angles * info_u.angles, info_u.groups)
...
cdef double[:,:,:,:] multigroup_tr_bdf2(double[:,:]& xs_total_u, double[:,:]& xs_total_c, \
        double[:,:,:]& xs_scatter_u, double[:,:,:]& xs_scatter_c, double[:]& velocity_u, \
        double[:]& velocity_c, double[:]& external_u, double[:]& boundary_xu, \
        double[:]& boundary_yu, int[:,:]& medium_map, double[:]& delta_x, \
        double[:]& delta_y, double[:]& angle_xu, double[:]& angle_xc, \
    ...
    flux_ell_x = tools.array_4d(info_u.cells_x + 1, info_u.cells_y, \
                               info_u.angles * info_u.angles, info_u.groups)
    flux_ell_y = tools.array_4d(info_u.cells_x, info_u.cells_y + 1, \
                               info_u.angles * info_u.angles, info_u.groups)
    flux_gamma = tools.array_4d(info_u.cells_x, info_u.cells_y, \
                               info_u.angles * info_u.angles, info_u.groups)

    # Combine last time step and source term
    q_star = tools.array_1d(info_u.cells_x * info_u.cells_y * info_u.angles \
    ...
    for step in tqdm(range(info_u.steps), desc="TR-BDF2", ascii=True):
        ...
        tools._time_source_star_cn(flux_ell_x, flux_ell_y, flux_u, \
                        ...
                        external_u[qq1::qq2], external_u[qq1::qq2], \
                        medium_map, delta_x, delta_y, angle_xu, angle_yu, \
